  ...
  private onGeneratorSuccess(
    ...
    let selectedWorkspace: string =
      type === "files" || type === "module" || ignoreGen
        ...
        : this.getWsConfig(this.SELECTED_WORKSPACE_CONFIG_PROP);
  ...
  private async createGeneratorPromptQuestions(
    ...
    if (vscodeInstance) {
      ...
      if (
        ...
        this.forceNewWorkspace = false;
        selectedWorkspaceConfig = this.getWsConfig(
    ...
    if (_.includes(genFilter.types, GeneratorType.project)) {
      const defaultPath = this.getWsConfig(this.TARGET_FOLDER_CONFIG_PROP)
        ? this.getWsConfig(this.TARGET_FOLDER_CONFIG_PROP)
        : this.getCwd();
